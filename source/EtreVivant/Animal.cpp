...
Animal::Animal(double energie, const double energiePourReproduction, const double geneVitesseManger, const double geneCote, const double geneVitesse, const sf::Texture & image, float x, float y, std::vector<Objet*> & objets, std::string proprietaire, sf::Int8 numero) :
...
{
    changerEnergie(-m_tempsEcoule/40.0);//vieillesse
...
//        std::cout<<"lolo"<<std::endl;
    ...
    /*if(m_seDeplace) {*//*std::cout<<"Je me déplace :) :)"<<std::endl;*/changerEnergie(-m_geneVitesse/15*m_tempsEcoule);/*}*/
...
//        std::cout<<(int)animal->type()<<std::endl;
        ...
        m_objets.push_back(new Animal(15.0, m_energiePourReproduction, geneVitesseManger, geneCote,geneVitesse, *texture(), x_, y_, m_objets, m_proprietaire));
